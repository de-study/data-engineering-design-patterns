...
──────────────────────────────────────────────────────────────────────────────
  적재시각 | 새 입력(증분) | 백필 직전 테이블        | 출력(소비자가 보는 값)
  -------- | ------------- | ---------------------- | -----------------------
...
```
  ...
  **무엇이 잘못됐나 — 08:00 스냅샷 비교:**
  ...
  백필이 08:00 을 다시 처리한 직후, 소비자는 **그 시점에 존재한 적도 없는 행 `C, M, N, O` 까지 섞인 데이터** 를 보게 됨.
...
```
[Transactional Writer 흐름 + 소비자 가시성]  Figure 4-9
──────────────────────────────────────────────────────────────────────
...
──────────────────────────────────────────────────────────────────────
  ...
  결과: 첫 MERGE 의 성공분도 DB 에 반영되지 않음
        → 소비자는 "둘 다 없음" 만 봄 (절반만 들어간 상태는 존재하지 않음)
