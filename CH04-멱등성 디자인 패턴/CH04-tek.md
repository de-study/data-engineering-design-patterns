...
대표적으로 자동 재시도(automatic retries) 가 데이터 정합성을 직접 흔듦
— 재시도된 task/job 은
...
> **참고 사항 — 멱등성(Idempotency) 한 줄 정의**
> `absolute(-1) == absolute(absolute(absolute(-1)))`
> — 몇 번 호출해도 같은 결과를 주는 함수의 성질.
> 데이터 엔지니어링에서는 **"같은 잡을 몇 번 돌려도 일관된 출력"** 을 보장하는 것
> — 중복이 아예 없거나, 있더라도 "식별 가능한 중복" 으로 남음.
...
멱등성 패턴의 첫 패밀리는 **data removal** 시나리오를 다룸
— "기존 데이터를 지운 뒤 새 데이터를 쓴다" 는 가장 단순한 접근.
다만 큰 데이터셋에서는 비용이 큼.
...
> **참고 사항 — `TRUNCATE TABLE table_a` ≒ `DELETE FROM table_a`**
> 의미상 둘 다 모든 행을 제거. 그러나 내부 동작이 다름
> — `TRUNCATE` 는 테이블 스캔을 하지 않으므로 **metadata operation** 으로 분류됨.

핵심 발상: 데이터셋을 monolithic 한 단일 테이블로 보지 말고, **여러 물리적 분할 테이블을 하나의 논리 단위로 노출** 하는 것.
예
— 1년치 visits 를 weekly 테이블 52개 + 단일 view 로 노출.
...
- 완화책
— **freezing step**: 일정 기간이 지난 mutable idempotent 테이블을 immutable 한 더 큰 단위로 합치기 (예: weekly → monthly 또는 yearly).
...
- 단, "새 required field 추가" 시나리오는 예외
— 어차피 과거 run 을 replay 하면서 새 필드가 자동 채워지므로 패턴 안에서 처리 가능.
...
> 또 다른 흔한 사고
> — daily granularity 로 운영하다 BigQuery 의 **테이블당 4,000 partition 한도** 에 11년차쯤 도달.
...
- Apache Spark
— `save mode` 옵션 (`overwrite`) 으로 writer 설정.
- Apache Flink
— write mode properties.
- 프레임워크가 알아서 기존 파일을 정리하고 새로 씀.
- **Delta Lake** 는 선택적 덮어쓰기 (`replaceWhere`) 도 지원
— 조건에 매칭되는 부분만 덮어쓰기.

**SQL 직접 사용 시**
- `DELETE FROM` + `INSERT INTO`
— 가장 익숙한 조합. 단순하지만 두 번의 동작이 필요.
- `INSERT OVERWRITE`
— 더 간결. 한 문장에 전체 테이블을 INSERT 부분의 레코드로 교체. 단점: row 단위 selective 덮어쓰기는 안 됨.
- 데이터 스토어의 native loading command
— 예: BigQuery 의 `LOAD DATA OVERWRITE`. 그 외는 보통 `TRUNCATE TABLE` 선행 필요.

> **참고 사항 — Not Only List of Columns**
...
- 데이터셋이 크고 **partitioned 되어 있지 않으면** overwrite 자체가 느려짐
— 시간이 갈수록 처리량이 줄어드는 구조.
- 완화책
— partitioning 같은 storage optimization 적용. 덮어쓸 데이터 양 자체를 줄이면 교체가 빨라짐.
...
- table file format, RDBMS 에서 발생
— 사용자 `SELECT` 에는 안 보이지만 dead row 가 디스크에 남음.
...
> 또 다른 사고
> — 100GB Iceberg 테이블에 일일 `INSERT OVERWRITE` 만 돌리고 `VACUUM` 을 한 번도 안 돌려서, 6개월 뒤 retention 안에 누적된 옛 스냅샷이 **30 TB** 까지 부풀어 객체 저장소 비용이 폭증.
...
대표적으로 **증분 갱신 데이터셋(updated incremental dataset)** 이 문제
— 데이터 제공자가 매 버전마다
...
> **참고 사항 — 전체 데이터셋이면 그냥 Overwrite 가 쉬움**
> 완전한 데이터셋을 매번 받는다면 덮어쓰기 패턴(#16/#17)이 더 단순함
> — 그냥 지우고 새로 쓰면 끝.
...
> **참고 사항 — `is_deleted` 플래그는 INSERT 절에도 필요**
...
책의 **Figure 4-4** 가 첫 실행(target 이 비어 모든 행이 `WHEN NOT MATCHED` 로 빠지는 상황)에서 이 차이를 보여줌.

```
[Figure 4-4 재현] 첫 실행 시 INSERT 절의 is_deleted 조건 유무 비교
──────────────────────────────────────────────────────────────────────
■ is_deleted 조건 없는 INSERT  (잘못된 경우)
   병합할 입력                        결과 테이블
     Record 1 (deleted) ──┐
     Record 2           ──┼─►  전부 NOT MATCHED → 전부 INSERT
     Record 3           ──┘        Record 1 (deleted)  ← 삭제 레코드가 그대로 적재 ✗
                                    Record 2
                                    Record 3
   ⇒ 이후 입력에 Record 1 이 다시 안 오면 MATCHED 도 안 돼 영원히 못 지움

■ is_deleted = false 조건 있는 INSERT  (올바른 경우)
   병합할 입력                        결과 테이블
     Record 1 (deleted) ──╳ (is_deleted=true → INSERT 제외)
     Record 2           ──┬─►  Record 2
     Record 3           ──┘    Record 3
   ⇒ 삭제 표시된 Record 1 은 처음부터 들어가지 않음 ✓
──────────────────────────────────────────────────────────────────────
```
...
- **I/O — data-based 패턴의 비용**
  ...
  - 완화책
  — 모던 DB/테이블 파일 포맷은 메타데이터 레이어에서 영향받는 레코드를 먼저 찾아 **불필요한 파일 스캔을 skip**.

- **Incremental datasets with backfilling — 백필 중 일시적 불일치 (Merger 의 가장 중요한 함정)**

  **핵심 원인** — Merger 는 입력으로 들어온 **증분만 `MERGE`** 하고, **입력에 없는 행은 절대 건드리지 않음**.
  정상 전진 실행에서는 행이 자연스럽게 쌓여 문제없지만, **백필은 "테이블의 현재(가장 최근) 상태" 에서 시작** 해 과거 증분을 다시 적용함.
  그래서 백필 시작 시점 **이후** 에 추가된 행들이 그대로 남아 **과거 시점의 스냅샷을 오염** 시킴.
  아래 책의 예시(Table 4-1 → 4-2)를 단계별로 따라가면 이 현상이 명확해짐.

  **(1) 정상 실행 흐름 — Table 4-1**

  매 적재 시각마다 "새 입력(증분)" 만 기존 테이블에 `MERGE`. `U`=update, `D`=delete(soft delete).

```
[Table 4-1] 정상 실행 — 증분을 시각순으로 MERGE
────────────────────────────────────────────────────────────────
  적재시각 | 새 입력(증분) | 출력 테이블(MERGE 결과)
  -------- | ------------- | -----------------------
  07:00    | A             | A
  08:00    | A-U, B        | A-U, B
  09:00    | B-D, C        | A-U, C
...
단계별 동작:
  07:00 : A 삽입                          ⇒ { A }
  08:00 : A→A-U 갱신, B 삽입              ⇒ { A-U, B }
  09:00 : B 삭제(soft delete), C 삽입     ⇒ { A-U, C }
  10:00 : M, N, O 삽입                    ⇒ { A-U, C, M, N, O }
────────────────────────────────────────────────────────────────
→ 10:00 시점 최종 상태 = { A-U, C, M, N, O } : 정상이고 일관됨
```

  여기까지는 문제가 없음. 각 시각의 증분이 차곡차곡 반영되어, 09:00 에 삭제된 `B` 는 최종 상태에서 사라지고
  10:00 에 추가된 `M, N, O` 가 정상적으로 들어가 있음.

  **(2) 08:00 부터 백필(replay) — Table 4-2**

  이제 08:00 run 에 버그가 발견되어 **08:00 부터 다시 돌린다고** 가정. 단, 테이블을 되돌리지 않고 증분만 그대로 재적용함.
  문제는 **백필이 시작되는 시점의 테이블이 이미 10:00 까지 반영된 최신 상태** `{ A-U, C, M, N, O }` 라는 점.

```
[Table 4-2] 08:00 부터 백필 — 테이블을 되돌리지 않고 증분만 재적용
──────────────────────────────────────────────────────────────────────────────
  적재시각 | 새 입력(증분) | 백필 직전 테이블        | 출력(컨슈머가 보는 값)
  -------- | ------------- | ---------------------- | -----------------------
  08:00    | A-U, B        | A-U, C, M, N, O        | A-U, B, C, M, N, O   ⚠
  09:00    | B-D, C        | A-U, B, C, M, N, O     | A-U, C, M, N, O
  10:00    | M, N, O       | A-U, C, M, N, O        | A-U, C, M, N, O
──────────────────────────────────────────────────────────────────────────────
단계별 동작 (출발점 = 최신 상태 { A-U, C, M, N, O }):
  08:00 재적용 : A-U 갱신 + B 삽입
                 → C, M, N, O 는 입력에 없어 그대로 잔류
                 ⇒ { A-U, B, C, M, N, O }   ← 정상 run 의 08:00 상태 { A-U, B } 와 다름!
  09:00 재적용 : B 삭제 + C 갱신             ⇒ { A-U, C, M, N, O }
  10:00 재적용 : M, N, O 갱신               ⇒ { A-U, C, M, N, O }   ← 이제야 정상 복귀
──────────────────────────────────────────────────────────────────────────────
```

  **무엇이 잘못됐나 — 08:00 스냅샷 비교:**
  - 정상 run 의 08:00 직후 상태: `{ A-U, B }`
  - 백필 중 08:00 직후 상태: `{ A-U, B, C, M, N, O }`

  백필이 08:00 을 다시 처리한 직후, 컨슈머 (읽는 쪽)는 **그 시점에 존재한 적도 없는 행 `C, M, N, O` 까지 섞인 데이터** 를 보게 됨.
  더 위험한 건 `B` — 정상 타임라인에서 `B` 는 09:00 에 삭제되어 최종 상태엔 없어야 하는데,
  백필 도중(08:00 재적용 ~ 09:00 재적용 사이)에는 `B` 가 **다시 살아나** 다른 미래 행들과 공존함.
  즉 **어느 시점에도 정상적으로 존재한 적 없는 "잡종 스냅샷"** 이 백필이 끝날 때까지 일시적으로 노출됨.
  백필이 10:00 까지 모두 끝나야 비로소 테이블이 `{ A-U, C, M, N, O }` 로 정상 수렴함.

  **(3) 완화책 — 복원(restore) 후 재적용**

  근본 해법은 **재적용 전에 테이블을 "첫 replay 시점 직전" 상태로 되돌리는 것**.
  08:00 부터 백필한다면, 08:00 증분을 적용하기 **전에** 테이블을 07:00 종료 상태 `{ A }` 로 롤백함.

```
복원(restore) 을 적용한 백필 — 07:00 종료 상태 { A } 로 롤백 후 재적용
────────────────────────────────────────────────────────────────
  롤백      : 테이블을 07:00 종료 상태 { A } 로 되돌림
  08:00 재적용 : A→A-U 갱신, B 삽입   ⇒ { A-U, B }          (정상 run 과 동일!)
  09:00 재적용 : B 삭제, C 삽입        ⇒ { A-U, C }          (정상 run 과 동일!)
  10:00 재적용 : M, N, O 삽입          ⇒ { A-U, C, M, N, O } (정상 run 과 동일!)
────────────────────────────────────────────────────────────────
```

  이렇게 하면 백필 중간 스냅샷도 정상 run 과 똑같아져 "잡종 스냅샷" 이 사라짐.
  다만 이 "롤백 → 재적용" 이 가능하려면 **버전 관리(time travel) 기능** 이 필요함(테이블 파일 포맷의 versioning 등).
  그리고 이 순간 stateless 였던 Merger 가 상태(state)를 가진 패턴으로 바뀜 → 바로 다음 **#19 Stateful Merger** 패턴이 이 복원 메커니즘을 정식으로 다룸.

> **참고 사항 — 데이터 제공자의 실수는 백필이 답이 아님**
...
> 또 하나
> — soft delete 의 INSERT 절에서 `is_deleted = false` 조건을 빠뜨리면, 최초 적재 때 이미 삭제된 레코드가 테이블에 들어가
...
데이터셋을 복원하려면 병합만으로는 부족함. Merger 는 **병합(merge) 동작 자체에만** 집중하기 때문.
**추가 상태 테이블(state table)** 로 데이터 복원 능력을 더한 것이 Stateful Merger.

**(1) 워크플로 — 기존 MERGE 앞뒤로 단계 2개 추가 (Figure 4-5)**

```
[Stateful Merger 워크플로]
──────────────────────────────────────────────────────────────
   ① 복원(restore)   →   ② MERGE   →   ③ 상태 테이블 갱신
  (백필일 때만 동작)        (병합)        (새 버전을 실행시각과 함께 기록)

  ① : 백필 모드면 테이블을 옛 버전으로 롤백, 정상 run 이면 그냥 통과
  ② : 기존 Merger 와 동일하게 증분을 MERGE → 새 테이블 버전이 생성됨
  ③ : ②가 만든 버전 번호를 (실행시각 → 버전) 으로 상태 테이블에 저장
──────────────────────────────────────────────────────────────
```

**(2) 상태 테이블이 만들어지는 원리 — Table 4-3**

이해를 위해 **마지막 단계(③)부터** 보는 게 쉬움. `MERGE` 가 끝나면 병합 테이블의 **새 버전** 이 생기고,
종료 태스크가 그 버전 번호를 **현재 실행 시각** 과 묶어 상태 테이블에 기록함.

```
[상태 테이블 적재 원리]  Table 4-3
──────────────────────────────────────────────────────────────
  09:00 실행 → MERGE 가 테이블 버전 5 생성 → 상태 테이블에 (09:00, 5) 기록
  10:00 실행 → MERGE 가 테이블 버전 6 생성 → 상태 테이블에 (10:00, 6) 기록
...
복원(①)은 **백필 모드일 때만** 동작하고, 정상 run 이면 아무 것도 하지 않음.
오케스트레이터가 실행 컨텍스트로 모드(백필/정상)를 알려주면 그대로 쓰면 되지만,
그렇지 못하면 **상태 테이블만으로** 아래처럼 판정함.

```
[백필 감지 & 복원 판정]
──────────────────────────────────────────────────────────────
  1) 상태 테이블에서 "이전 run 이 만든 버전" 조회
       └─ 없음 → 첫 실행 또는 "첫 실행을 백필" 하는 경우
                 → 되돌릴 기준이 없으므로 TRUNCATE TABLE 후 바로 MERGE
       └─ 있음 → 2) 로 진행

  2) (이전 run 의 버전)  vs  (테이블의 현재 최신 버전) 비교
       └─ 같음 → 정상 run     → 복원 없이 바로 MERGE
       └─ 다름 → 백필 모드     → 테이블을 "이전 run 의 버전" 으로 restore 후 MERGE
──────────────────────────────────────────────────────────────
직관: "내 직전 실행이 만든 버전" 과 "지금 테이블의 최신 버전" 이 어긋났다는 건,
      내 뒤에 다른(미래) 실행들이 테이블을 더 바꿔놨다는 뜻 = 지금은 백필 상황.

예) Table 4-3 에서 09:00 을 다시 실행하면
    이전 run(08:00) 버전 = 4,  최신 버전 = 6  →  4 ≠ 6  →  백필 → 버전 4 로 복원 후 MERGE
```

**(4) 시나리오별 동작 추적 — Table 4-4 → 4-5**

일일 잡이 4회(10-05 ~ 10-08) 정상 실행되어 상태 테이블이 아래(Table 4-4)와 같은 상황을 가정.
세 가지 대표 시나리오를 따라가면 판정 로직이 어떻게 적용되는지 분명해짐.

```
[시나리오별 동작]
──────────────────────────────────────────────────────────────
  상태 테이블(Table 4-4)            테이블 현재 최신 버전 = 4
    2024-10-05 | 1
    2024-10-06 | 2
    2024-10-07 | 3
    2024-10-08 | 4

  ┌ 시나리오 1) 정상 전진 — 2024-10-09 실행
  │    이전 run(10-08) 버전 = 4,  최신 버전 = 4  →  같음 = 정상
  │    복원 없이 MERGE → 새 버전 5 생성, (10-09, 5) 기록
  │
  ├ 시나리오 2) 첫 실행을 백필 — 2024-10-05 재실행
  │    이전 run(10-04) 버전 = 없음  →  TRUNCATE TABLE 후 MERGE
  │    (되돌릴 기준 버전이 없으니 빈 테이블에서 다시 시작)
  │
  └ 시나리오 3) 중간 시점 백필 — 2024-10-07 재실행
       이전 run(10-06) 버전 = 2,  최신 버전 = 4  →  다름 = 백필
       테이블을 버전 2 로 restore → 10-07 증분 MERGE → 새 버전 5 기록
──────────────────────────────────────────────────────────────
시나리오 3 이후 상태 테이블 (Table 4-5):
    2024-10-05 | 1
    2024-10-06 | 2
    2024-10-07 | 5    ← 3 에서 5 로 갱신됨
    2024-10-08 | 4

  이어서 오케스트레이터가 2024-10-08 도 자동으로 백필:
    이전 run(10-07) 버전 = 5(방금 갱신됨),  최신 버전 = 5  →  같음 = 정상 run 으로 처리
    → 복원 없이 MERGE 만 수행 → 테이블이 정상 상태로 매끄럽게 수렴
──────────────────────────────────────────────────────────────
```

핵심은 **시나리오 3 → 그다음 10-08 백필** 흐름.
10-07 을 복원·재처리해 새 버전(5)을 만들면, **바로 다음 백필인 10-08 입장에서는 "이전 run(10-07)이 만든 버전 = 최신 버전"** 이 되어
자동으로 정상 run 분기로 들어감. 즉 한 시점만 복원하면 그 뒤 시점들은 별도 복원 없이 차례로 정상 수렴함.

**(5) 버저닝 없는 데이터 스토어에서의 변형 — Figure 4-6**

테이블 파일 포맷처럼 버전 관리가 없는 스토어에서는, **버전 대신 원시 데이터 이력 테이블** 로 같은 효과를 냄.

```
[버저닝 없는 스토어 변형]
──────────────────────────────────────────────────────────────
  테이블 버전 대신, 원시 데이터 테이블(devices_history)에
  execution_time 컬럼과 함께 모든 원시 데이터를 누적 적재.

  백필 감지 — "미래 실행 시각의 레코드가 이미 있는가?" (Example 4-9)
    SELECT CASE WHEN COUNT(*) > 0 THEN true ELSE false END
    FROM dedp.devices_history WHERE execution_time > '{{ ts }}'

       └─ true(미래 행 존재) = 백필 →
            1) execution_time >= '{{ ts }}' 인 행 삭제
               (= 마지막 정상 버전에 해당하는 행만 남김)
            2) devices_history 를 Windowed Deduplicator 로 메인 테이블 재구성
            3) 현재 execution_time 입력을 적재 후 메인 테이블과 MERGE
──────────────────────────────────────────────────────────────
```

감지(`>`)와 삭제(`>=`)의 부등호 차이에 주의 — 미래 행이 **하나라도** 있으면 백필로 보고,
삭제는 **현재 실행 시각 이상** 의 행을 모두 걷어내 마지막 정상 상태로 되돌린 뒤 재구성함.
...
- **Vacuum operations — retention 만료가 복원 한계를 정함**
  ...
  - 완화책
  — retention 기간을 늘리면 백필 가능 범위가 늘지만 **저장 비용 증가**. 또는 "retention 너머는 백필 불가" 를 수용.
...
- **Metadata operations — compaction 이 만드는 버전 점프**
  ...
  - 완화책
  — 버전이 1씩 증가한다는 가정하에, **"현재 실행 시각의 버전 − 1"** 을 복원 버전으로 사용.
...
> 또 다른 함정
> — 야간에 compaction 잡이 돌아 버전이 1 점프했는데 복원 로직이 "이전 버전" 을 그대로 쓰면,
...
왜 event time 으로 키를 만들면 위험한지, 책의 **Figure 4-8** 이 구체적인 값으로 보여줌.
세션 키는 보통 `user_id` + **가장 이른 활동 시각** 으로 만드는데(예: `1_09:56`),
event time 으로 최소값을 잡으면 late 데이터가 그 최소값을 끌어내려 키가 바뀜.
반면 append time(브로커에 물리적으로 기록된 시각)은 "늦게 도착 = 더 큰 값" 이라 최소값을 절대 바꾸지 못함.

```
[Figure 4-8 재현] event time 기반 키가 late 데이터로 깨지는 과정
──────────────────────────────────────────────────────────────────────────────
■ 10:00, 잡 업데이트 전 (최초 실행)
   입력 저장소(user_id=1)            세션 생성 잡        출력
     append=09:58  event_time=09:57  ──┐
     append=09:57  event_time=09:56  ──┴─►  키 = 1 + min(event)
                                            = 1 + 09:56      →  session_id: 1_09:56

■ 10:02, 잡 업데이트 후 (런타임 에러로 재시작) — late 레코드가 끼어듦
   입력 저장소(user_id=1)            세션 생성 잡        출력
     append=09:59  event_time=09:55  ──┐  ← late (가장 늦게 append, 그러나 event 는 가장 빠름)
     append=09:58  event_time=09:57  ──┤
     append=09:57  event_time=09:56  ──┴─►  키 = 1 + min(event)
                                            = 1 + 09:55      →  session_id: 1_09:55  ✗
──────────────────────────────────────────────────────────────────────────────
결과: 기존 1_09:56 옆에 1_09:55 가 새로 생성 → 한 사용자의 세션이 둘로 쪼개짐 = 멱등성 깨짐
```

같은 입력에 **append time** 으로 키를 잡으면 어떻게 달라지는지 비교하면 해법이 분명해짐.

```
[해법] append time(불변) 기반 키 — 같은 입력, 같은 키 유지
──────────────────────────────────────────────────────────────────────────────
  최초 실행 : min(append) = 09:57            → session_id: 1_09:57
  재시작    : late 레코드 append=09:59 추가
              min(append) = 09:57 (그대로!)  → session_id: 1_09:57   ✓ 키 유지
──────────────────────────────────────────────────────────────────────────────
late 레코드는 "물리적으로 가장 늦게 append" 되므로 min(append) 을 끌어내리지 못함.
→ 배치·스트리밍 모두 동일하게 키가 1_09:57 로 고정됨.
```

이 전략은 **key-value 스토어뿐 아니라 파일/파티션/테이블 컨테이너에도** 그대로 적용됨.
핵심은 동일하게 **불변 속성으로 이름(키)을 짓는 것**.

```
[키 전략의 다른 적용 — 파일/파티션/테이블 이름을 실행 시각으로]
─────────────────────────────────────────────────────────────────
  2024-11-20 실행 → 파일명 "20_11_2024"
  2024-11-21 실행 → 파일명 "21_11_2024"

  같은 실행일을 replay → 같은 파일명 하나만 다시 생성 (중복 없음)
  → 파티션·테이블도 동일 원리: "하루치 = 테이블 하나" 식으로 불변 이름 부여
─────────────────────────────────────────────────────────────────
```
...
- **Database dependent — DB 종류에 따라 적용 가능성이 다름**
  ...
  - **Apache Kafka** — 키를 지원하지만 append-only 로그라 **삽입 시점에 중복 제거를 하지 않음**. 대신 **비동기 compaction** 으로 나중에 정리 → 그 사이 컨슈머는 중복을 볼 수 있음(단 같은 키라 구분은 쉬움).
...
> **참고 사항 — Kafka 의 timestamp 속성**
> Kafka 프로듀서가 append time 을 외부에서 지정할 수도 있지만, 패턴 관점에서는 **브로커가 완전히 통제하는 값** 보다 덜 신뢰됨.
> 토픽 설정은 `log.message.timestamp.type` 속성으로 확인 가능.

#### 구현 예시 (Examples)

아래 예시 1~5 는 **Kafka 방문 이벤트 → Spark Structured Streaming 세션화 → ScyllaDB 적재** 라는 하나의 파이프라인을 조각낸 것.
전체 흐름을 먼저 보면 각 코드 조각이 어디에 해당하는지 잡힘.

```
[Keyed Idempotency 전체 흐름 — Spark Structured Streaming]
──────────────────────────────────────────────────────────────────────
  Kafka(visit 이벤트)
    value(JSON) + timestamp(=append time)
        │
        ▼  [예시 3] value 파싱 + append_time 추출 + user_id 로 groupBy + watermark
  groupBy(user_id) + mapGroupsWithState
        │
        ├─ [예시 5] 상태 누적: 그룹의 "가장 이른 append_time" 을 첫 버전에 고정
        │            (이미 상태가 있으면 기존 append_time 유지 → 재처리에도 불변)
        │
        ▼  [예시 4] 윈도우 만료(hasTimedOut) → session_id = hash(min_append_time)
  세션 레코드 { session_id, user_id, pages }
        │
        ▼  [예시 1·2] ScyllaDB sessions 테이블에 적재
  PRIMARY KEY(session_id, user_id) → 같은 키면 덮어쓰기 = 정확히 한 번
──────────────────────────────────────────────────────────────────────
멱등성의 원천: session_id 가 불변 append_time 에서만 파생됨
              → 몇 번 재처리해도 같은 키 → ScyllaDB 에 한 행만 남음
```
...
다른 브로커도 **속성명만 다를 뿐** 동일하게 구현됨. 예
— Amazon Kinesis 는 읽기 부분만 바꿔 `approximateArrivalTimestamp` 를 `append_time` 으로 매핑:
```python
...
| Apache Kafka | append-only, 삽입 시 중복 제거 없음 | 비동기 compaction 전까지 중복 노출(같은 키) |
...
```
[Transactional Writer 흐름 + 컨슈머 가시성]  Figure 4-9
──────────────────────────────────────────────────────────────────────
  Producer:  BEGIN ──── write ──── write ──── write ──┬── COMMIT
                                                       └── (오류) ROLLBACK

  데이터 상태: │←──────── private (트랜잭션 범위 안) ────────→│← public →│

  Consumer:   │←──────── 아무 것도 못 봄 ───────────────────→│← 전체 노출
──────────────────────────────────────────────────────────────────────
  COMMIT 성공 → 그 순간 모든 레코드가 한꺼번에 보임 (부분 노출 없음)
  ROLLBACK / 잡 실패 → 쓰던 레코드는 폐기되거나 orphan 으로 남아 안 보임
  (단, reader 가 read uncommitted 격리면 commit 전에도 보일 수 있음 = dirty read)
```
...
```
[구현 모델 3가지 — 어디서 commit 이 일어나나]
──────────────────────────────────────────────────────────────────────
  A) standalone / ELT (DW 가 관리)
       BEGIN ─ SQL 처리 ─ COMMIT         ← DB 가 선언적으로 트랜잭션 관리
       (내부는 분산이어도 트랜잭션 경계는 DB 가 책임)

  B) 분산 ETL — 로컬(task) 트랜잭션
       task1: BEGIN─write─COMMIT   task2: BEGIN─write─COMMIT   ...
       └ task 별로 따로 commit → 잡이 재시작되면 이미 commit 된 task 가 또 write ⚠

  C) 분산 ETL — 잡 전체 트랜잭션
       JOB BEGIN ─ [task1 write | task2 write | ...] ─ JOB COMMIT
       └ 모든 task 가 끝나야 한 번에 commit → 강한 보증, 구현 난도 ↑
         (Spark+Delta: commit log 에 엔트리 생성 시 commit. 실패 시 데이터 파일 수동 정리)
──────────────────────────────────────────────────────────────────────
멱등성 강도:  A ≈ C  >  B   (B 는 부분 commit 이 남아 재시작·백필에 취약)
```

**멱등성 범위 주의** — all-or-nothing 은 **현재 run 안에서만** 멱등.
백필하면 writer 가 **새 트랜잭션** 을 열어 같은 레코드를 또 insert 함. 즉 멱등성은 트랜잭션 단위로만 보장됨.

> **참고 사항 — Read Uncommitted = dirty reads**
...
단, 분산 처리 도구와의 통합은 제각각
— 테이블 파일 포맷은 Flink/Spark 가 잘 지원하지만, **Kafka 트랜잭션 프로듀서는 Flink 에서만** 가능.
...
이 예시의 핵심은 **"첫 MERGE 가 성공처럼 보여도 commit 에 도달 못 하면 무효"** 라는 점.

```
[Example 4-21 동작 — 부분 성공은 없다]
──────────────────────────────────────────────────────────────────────
  (암묵적 BEGIN)
    파일1 로드 → MERGE devices            ✓ 성공 (아직 private)
    파일2 로드 → MERGE devices            ✗ 실패 (컬럼 길이 초과)
  COMMIT  ─────────────────────────────── ✗ 여기 도달 못 함
  ──────────────────────────────────────────────────
  결과: 첫 MERGE 의 성공분도 DB 에 반영되지 않음
        → 컨슈머는 "둘 다 없음" 만 봄 (절반만 들어간 상태는 존재하지 않음)
  ※ 테이블 파일 포맷도 동일: commit 파일이 없으면 쓴 파일들은 읽히지 않음
──────────────────────────────────────────────────────────────────────
```
...
Kafka 도 SQL/테이블 포맷과 똑같이 "commit 전까지 비공개" 로 동작함. 다만 commit 시점이 **Flink checkpoint** 에 묶여 있는 게 특징.

```
[Kafka 트랜잭션 프로듀서 흐름 + timeout 함정]
──────────────────────────────────────────────────────────────────────
  프로듀서 → 파티션에 "트랜잭션 시작" 특수 메시지 전송
          → 레코드 전송 (컨슈머에는 uncommitted 로 표시, 아직 안 읽힘)
          → commit: 새 메타데이터 이벤트로 pending 트랜잭션 확정 → 그때 공개

  Flink 연동:  checkpoint 완료 == commit 트리거
  ──────────────────────────────────────────────
  transaction.timeout.ms 안에 checkpoint 가 끝나야 함
     timeout=30s, checkpoint 평균=45s  →  트랜잭션 만료 → commit 실패 → 출력 비어버림
     ⇒ transaction.timeout.ms 를 checkpoint 소요시간보다 넉넉히 크게 잡을 것
──────────────────────────────────────────────────────────────────────
```
...
> **이미 commit 된 task 의 출력이 다시 write** 되어 같은 레코드가 2배로 들어감
> — 트랜잭션 단위로는 "정상" 이라 에러도 안 남.
> 또 하나
...
- **#22 Proxy** — 데이터는 매번 새 위치에 **한 번만 write**(write-once) 하여 과거 버전을 모두 보존하되,
  컨슈머 (읽는 쪽)에게는 **중간 계층(view·manifest)** 을 통해 항상 최신 데이터 하나만 단일 지점으로 노출.
...
> 핵심은 **컨슈머 (읽는 쪽)와 실제 물리 저장소 사이에 한 겹의 간접 계층(proxy)** 을 두는 것.
...
- **결정적 제약**: 데이터셋이 **immutable(한 번만 쓰기, write-once)** 이어야 함.
  동시에 컨슈머 (읽는 쪽)에게는 **언제나 최신 테이블 하나만, 단일 지점에서** 노출해야 함
...
컨슈머 (읽는 쪽)가 최신 데이터에 접근할 **하나의 진입점** 을 만듦. 구현은 세 가지.
...
- **구현 2 — Manifest 기반**: view 를 지원하지 않는 저장소라면, 컨슈머 (읽는 쪽)가 처리해야 할 파일 위치/목록을
...
──────────────────────────────────────────────────────────────────────
 공통점: 컨슈머는 늘 "하나의 접근점" 만 보지만, 그 뒤로 과거 버전이 모두 살아 있음
```
...
refresh_view = PostgresOperator(  # ...
    sql='/sql/refresh_view.sql'                   # (2) 컨슈머용 view 를 최신 내부 테이블로 갱신
)
...
──────────────────────────────────────────────────────────────────────
  ...
  수요일 run → devices_internal_20240703_000000   (보존, 쓰기권한 제거)
            ...
            ┌──────────────────────────────┐
   컨슈머 → │  view: dedp.devices          │ → 항상 "최신 한 개" 만 보임
            └──────────────────────────────┘
...
오류 관리(챕터 3)는 실패의 영향을 **처리 계층** 에서 완화할 뿐.
사이클을 완성하려면 **멱등성(idempotency)** 이 필요하고, 그게 이 챕터의 패턴들.
