...
#### Q1. emit은 언제되나 
> 상태저장세션화기에서 emit가 자동으로 된다는데 어떤프로세스로 ? 
> 세션시간계산해서 처리하는건 알겠는데 매 시간 상태 테이블에 있는 세션들의 시간 체크하고 계산하고 처리하나?

Spark은 **새 이벤트가 들어올 때마다** 워터마크를 갱신함.

```
[10:01] 이벤트 수신 → 워터마크 이동
[10:08] 이벤트 수신 → 워터마크 이동
[10:44] 이벤트 수신 → 워터마크 이동
         ↓
         워터마크가 10:44 근처로 이동
         → "V001 타이머가 10:18이었는데 워터마크가 넘었다"
         → hasTimedOut = True 발동
         → map_visits_to_session 자동 호출
         → emit
```
...
- 주기적으로 체크하는 게 아님
- **새 이벤트 도착 → 워터마크 갱신 → 만료된 세션 있으면 그때 처리**
- 즉, 새 이벤트가 안 들어오면 만료된 세션도 emit 안 됨
...
근데 10:44까지 다른 이벤트가 아무것도 안 들어오면?
→ 워터마크 안 움직임
→ V001 만료 안 됨
→ emit 안 됨
```

이게 스트리밍의 특성이야. **시계가 이벤트로 움직임.**



#### Q2. V001 이거를 뭐라고해?, 어떻게 생성되나 

| 용어         | 설명                                        |
| ---------- | ----------------------------------------- |
| `visit_id` | 세션을 식별하는 키. V001, V002 같은 값               |
| 세션 키       | visit_id랑 같은 말. state store에서 데이터를 찾는 기준  |
| 그루핑 키      | Spark에서 `.groupBy(visit_id)` 할 때 기준이 되는 키 |

visit_id 값(V001, V002)이 어떻게 생성되는지?

visit_id는 **이벤트 발생 시점에 클라이언트(브라우저/앱)가 생성**해서 Kafka로 보내는 값


**생성 흐름**
```
사용자가 브라우저에서 페이지 접속
→ 브라우저 JS가 visit_id 생성 (UUID 등)
→ 모든 클릭/페이지 이동에 같은 visit_id 붙여서 이벤트 전송
→ Kafka로 전달
→ Spark이 받아서 groupBy(visit_id)로 묶음
```


**Spark 입장에서는**
- visit_id를 직접 만들지 않음
- Kafka 이벤트 안에 이미 들어있는 값을 그루핑 키로 쓰는 것
```
Kafka 이벤트 예시:
{
  "visit_id": "V001",      ← 클라이언트가 생성해서 보낸 값
  "user_id": "user_A",
  "page": "/home",
  "event_time": "10:01"
}
