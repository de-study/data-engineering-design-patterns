...
|---|---|---|---|
| 네이티브 header/tag | 브로커 header, 파일 tag | 컨슈머가 안 읽으면 됨 | 브로커가 지원해야 함(Kinesis ✗) |
...
- **Scaling (확장성)**
  - 셔플은 확장성에도 영향. 어떤 노드가 reduce 를 다 끝냈어도 **fault tolerance 목적** 으로 계속 점유될 수 있음 —
    reduce 가 통째로 실패해 재시작될 때 reshuffle 을 피하기 위함. 실패가 없으면 그 노드는 처리가 끝날 때까지 회수되지 않음.
  - 이를 피하려면 **shuffle service** 사용 — 셔플 데이터만 저장·서빙하는 별도 compute 컴포넌트.
    한 노드가 더 안 쓰이면 잡 실행 중에도 즉시 회수 가능.
...
──────────────────────────────────────────────────────────────────────
 Distributed (#27)   입력 적재 ─► [노드별 부분집계] ─► ✗ shuffle(네트워크) ─► reduce
                     같은 key 가 여러 노드에 흩어져 있음 ⇒ 모으려면 셔플 필수

 Local (#28)         입력 적재 ─► [파티션 내 로컬 집계] ─► ✓ shuffle 없음 ─► 출력
...
- **Grouping keys (그룹 키)**
  - 정적 파티션 수 데이터 소스에서는 **모든 컨슈머가 하나의 grouping key 로직** 을 쓸 것을 전제함.
    같은 record 를 grouping key 마다 여러 곳에 써야 해서 쉽지 않음.
  - 예: 한 앱은 user 프로필 변경을 **변경 타입** 으로 묶는데 다른 컨슈머는 **user ID** 기반 집계를 한다면,
    둘 중 하나는 결국 **Distributed Aggregator** 를 택해야 함.
...
- **Input dataset storage (입력 데이터셋)** — 세션화 파이프라인에서 상관시킬 raw 이벤트.
  여기 상황의 시간별 분할 visits 가 들어감.
...
- **Pending sessions storage (대기 세션)** — 다음 실행에서 닫힐, 여러 파티션에 걸친 세션을 보관.
  완료 세션 저장소와 두 가지가 다름 — ① **비공개** 여야 함(내부 로직과 함께 진화. 컨슈머가 세부를 알 필요 없고, 알면 진화 유연성이 떨어짐),
...
- 해당 엔티티에 대기 세션이 없으면 **새 세션**.
- 읽은 입력의 새 데이터로 **복원된 세션(new data 있음)**.
- 새 데이터 없이 **복원된 세션(new data 없음)** — 만료 규칙에 따라 이번/다음 실행에서 곧 만료될 세션.
